...
class BathroomManager {
    ...
    Gender lastServedGender;  // Keep track of the last served gender
...
public:
    BathroomManager(int cap, int numThreads) : bathroom(cap), executor(numThreads), lastServedGender(Gender::NONE) {}
    ...
    void scheduleNextTask() {
        executor.execute([this] {
            ...
            // Round-robin scheduler: Alternate between genders if both queues have tasks
            if (lastServedGender == Gender::FEMALE && !menQueue.isEmpty()) {
                auto task = menQueue.pop();
                lastServedGender = Gender::MALE;
                lock.unlock();
                task();
            } else if (lastServedGender == Gender::MALE && !womenQueue.isEmpty()) {
                auto task = womenQueue.pop();
                lastServedGender = Gender::FEMALE;
                lock.unlock();
                task();
            } else if (!menQueue.isEmpty()) {
                auto task = menQueue.pop();
                lastServedGender = Gender::MALE;
                lock.unlock();
                task();
            } else if (!womenQueue.isEmpty()) {
                auto task = womenQueue.pop();
                lastServedGender = Gender::FEMALE;
                lock.unlock();
                task();
